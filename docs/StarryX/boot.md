...
xhal模块抽象了底层硬件与平台，StarryX从链接脚本的特定位置启动，跳转到特定arch的_start函数完成CPU与mmu初始化后跳转到硬件平台的rust_entry，完成对硬件中断、时钟、串口等的硬件初始化后跳转到xruntime模块进行组件初始化：
...
xruntime模块确保内核运行环境就绪并为宏内核扩展提供支撑，完成了kernel backbone的初始化过程。其启动流程包括以下步骤：
...
pub extern "C" fn rust_main(cpu_id: usize, dtb: usize) {
    ...
    xmm::init();      		    // 虚拟内存管理初始化
    xtask::init_scheduler();   // 多任务调度器初始化
    xdriver::init();           // 驱动初始化
...
fn main() {
    ...
    xkernel::fs::pseudofs::init_root();	// 初始化伪文件系统
    xkernel::fs::fd::init_stdio();    // 初始化FD表
    run_user_app()					// 运行用户程序                    
}
```
