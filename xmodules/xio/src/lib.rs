...
pub trait Read {
    ...
    fn read_exact(&mut self, mut buf: &mut [u8]) -> Result {
        ...
        if !buf.is_empty() {
            x_err!(UnexpectedEof, "failed to fill whole buffer")
        } else {
            Ok(())
        }
    }
    ...
    fn write_all(&mut self, mut buf: &[u8]) -> Result {
        while !buf.is_empty() {
            match self.write(buf) {
                Ok(0) => return x_err!(WriteZero, "failed to write whole buffer"),
                Ok(n) => buf = &buf[n..],
                Err(e) => return Err(e),
            }
        }
        ...
        match fmt::write(&mut output, fmt) {
            ...
            Err(..) => {
                ...
                if output.error.is_err() {
                    ...
                    x_err!(InvalidData, "formatter error")
                }
            }
        }
    }
    ...
    F: FnOnce(&mut Vec<u8>) -> Result<usize>,
{
    ...
    if core::str::from_utf8(&buf[old_len..]).is_err() {
        x_err!(InvalidData, "stream did not contain valid UTF-8")
    } else {
        Ok(ret)
    }
}
