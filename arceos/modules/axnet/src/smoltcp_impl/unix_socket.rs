...
impl UnixSocket {
    ...
    /// 创建已连接的 Unix Socket
    fn new_connected(local_addr: UnixAddr, peer_addr: UnixAddr, connection: ConnectionPair) -> Self {
        Self {
            state: AtomicU8::new(STATE_CONNECTED),
    ...
    pub fn peer_addr(&self) -> AxResult<UnixAddr> {
        match self.get_state() {
            STATE_CONNECTED => {
                Ok(unsafe { self.peer_addr.get().read() }.clone())
            }
    ...
    pub fn connect(&self, addr: UnixAddr) -> AxResult {
        self.update_state(STATE_CLOSED, STATE_CONNECTING, || {
            ...
            let server_socket = UnixSocket::new_connected(
                server_addr, 
                client_addr.clone(), 
                server_conn
            );
    ...
    fn poll_listener(&self) -> AxResult<PollState> {
        let local_addr = unsafe { self.local_addr.get().read() }.clone();
        let listen_table = LISTEN_TABLE.lock();
        let has_pending = listen_table.get(&local_addr)
