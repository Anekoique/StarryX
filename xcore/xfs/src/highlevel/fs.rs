...
xns::def_resource! {
    pub static FS_CONTEXT: xns::ResArc<xsync::Mutex<FsContext<xsync::RawMutex>>> = xns::ResArc::new();
}

#[cfg(feature = "thread-local")]
impl FS_CONTEXT {
    pub fn copy_inner(&self) -> xsync::Mutex<FsContext<xsync::RawMutex>> {
        xsync::Mutex::new(self.lock().clone())
    }
}
