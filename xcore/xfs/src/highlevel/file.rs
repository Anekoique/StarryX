...
use xio::SeekFrom;
use xvfs::{FileNode, FileNodeOps, Location, Metadata, VfsError, VfsResult};
...
fn vfs_error_to_axio(err: VfsError) -> xio::Error {
    match err {
        VfsError::EEXIST => xio::Error::AlreadyExists,
        VfsError::ENOTEMPTY => xio::Error::DirectoryNotEmpty,
        VfsError::EINVAL => xio::Error::InvalidInput,
        VfsError::EISDIR => xio::Error::IsADirectory,
        VfsError::ENOMEM => xio::Error::NoMemory,
        VfsError::ENOTDIR => xio::Error::NotADirectory,
        VfsError::ENOENT => xio::Error::NotFound,
        VfsError::EBADF => xio::Error::PermissionDenied,
        VfsError::ENOSPC => xio::Error::StorageFull,
        VfsError::ENOSYS | VfsError::EOPNOTSUPP => xio::Error::Unsupported,
        _ => xio::Error::Io,
    }
}

impl<M: RawMutex> xio::Read for FsFile<M> {
    fn read(&mut self, buf: &mut [u8]) -> xio::Result<usize> {
...
impl<M: RawMutex> xio::Write for FsFile<M> {
    fn write(&mut self, buf: &[u8]) -> xio::Result<usize> {
        if self.flags.contains(FileFlags::APPEND) {
    ...
    fn flush(&mut self) -> xio::Result {
        Ok(())
    }
}
impl<M: RawMutex> xio::Seek for FsFile<M> {
    fn seek(&mut self, pos: SeekFrom) -> xio::Result<u64> {
        let new_pos = (|| {
            Ok(match pos {
