...
unsafe extern "C" {
    fn __start_xns_resource();
    fn __stop_xns_resource();
}
...
///   `xns_resource` section, and the global namespace is constructed by the base
///   address of the section ([`XNamespace::global`]).
/// - Thread-local namespace: this namespace is per-thread, each thread should
///   call [`XNamespace::new_thread_local()`] to allocate a memory area as its
...
impl XNamespace {
    ...
    /// Returns the size of the `xns_resource` section.
    fn section_size() -> usize {
        __stop_xns_resource as *const () as usize - __start_xns_resource as *const () as usize
    }

    /// Returns the global namespace.
    pub fn global() -> Self {
        Self {
            base: __start_xns_resource as *const () as usize,
            alloc: false,
        }
    }
    ...
    pub fn new_thread_local() -> Self {
        ...
        let base = if size == 0 {
            ...
            let src = __start_xns_resource as *const u8;
...
pub unsafe fn current_namespace_base() -> *mut u8 {
    #[cfg(feature = "thread-local")]
    {
        crate_interface::call_interface!(XNamespaceIf::current_namespace_base)
    }
    #[cfg(not(feature = "thread-local"))]
    {
        XNamespace::global().base()
    }
}

/// Defines a resource that managed by [`XNamespace`].
///
/// Each resource will be collected into the `xns_resource` section. When
...
///
/// mod imp {
///     use xns::{XNamespace, XNamespaceIf};
///
///     struct ResArcImpl;
///
///     #[crate_interface::impl_interface]
///     impl XNamespaceIf for ResArcImpl {
///         fn current_namespace_base() -> *mut u8 {
///             XNamespace::global().base()
///         }
///     }
