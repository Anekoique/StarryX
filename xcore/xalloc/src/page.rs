...
impl GlobalPage {
    ...
    pub fn alloc() -> XResult<Self> {
        global_allocator()
            ...
            .map_err(alloc_err_to_x_err)
    }

    /// Allocate one 4K-sized page and fill with zero.
    pub fn alloc_zero() -> XResult<Self> {
    ...
    pub fn alloc_contiguous(num_pages: usize, align_pow2: usize) -> XResult<Self> {
        global_allocator()
...
const fn alloc_err_to_x_err(e: AllocError) -> XError {
    match e {
        AllocError::InvalidParam | AllocError::MemoryOverlap | AllocError::NotAllocated => {
            XError::InvalidInput
        }
        AllocError::NoMemory => XError::NoMemory,
    }
}
