...
use xconfig::{SMP, TASK_STACK_SIZE};
use xhal::mem::{VirtAddr, virt_to_phys};
...
pub fn start_secondary_cpus(primary_cpu_id: usize) {
    ...
    for i in 0..SMP {
        if xconfig::PLATFORM == "riscv64-visionfive2" && i == 0 {
            continue;
        }
        if i != primary_cpu_id && logic_cpu_id < SMP - 1 {
            ...
            xhal::mp::start_secondary_cpu(i, stack_top);
            logic_cpu_id += 1;
...
///
/// It is called from the bootstrapping code in [xhal].
#[unsafe(no_mangle)]
pub extern "C" fn rust_main_secondary(cpu_id: usize) -> ! {
    ...
    #[cfg(feature = "paging")]
    xmm::init_memory_management_secondary();

    xhal::platform_init_secondary();

    #[cfg(feature = "multitask")]
    xtask::init_scheduler_secondary();
