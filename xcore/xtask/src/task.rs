...
use xhal::arch::TaskContext;
#[cfg(feature = "tls")]
use xhal::tls::TlsArea;

use crate::task_ext::XTaskExt;
use crate::{WaitQueue, XCpuMask, XTask, XTaskRef};
...
impl TaskInner {
    ...
    ///
    /// Returns the cpu affinity mask of the task in type [`XCpuMask`].
    #[inline]
    pub fn cpumask(&self) -> XCpuMask {
        *self.cpumask.lock()
    }
    ...
    ///
    /// # Arguments
    /// `cpumask` - The cpu affinity mask to be set in type [`XCpuMask`].
    #[inline]
    pub fn set_cpumask(&self, cpumask: XCpuMask) {
...
impl TaskInner {
    fn new_common(id: TaskId, name: String) -> Self {
        Self {
            ...
            cpumask: SpinNoIrq::new(XCpuMask::full()),
    ...
    pub(crate) fn into_arc(self) -> XTaskRef {
        Arc::new(XTask::new(self))
    }
    ...
    fn current_check_preempt_pending() {
        ...
        if curr.need_resched.load(Ordering::Acquire) && curr.can_preempt(0) {
            ...
            // disable preemption here, because the xlog may cause preemption.
...
/// A wrapper of [`XTaskRef`] as the current task.
///
/// It won't change the reference count of the task when created or dropped.
pub struct CurrentTask(ManuallyDrop<XTaskRef>);

impl CurrentTask {
    pub(crate) fn try_get() -> Option<Self> {
        let ptr: *const super::XTask = xhal::cpu::current_task_ptr();
        if !ptr.is_null() {
            Some(Self(unsafe { ManuallyDrop::new(XTaskRef::from_raw(ptr)) }))
        } else {
            None
        }
    }
    ...
    /// Converts [`CurrentTask`] to [`XTaskRef`].
    pub fn as_task_ref(&self) -> &XTaskRef {
        &self.0
    }

    pub(crate) fn clone(&self) -> XTaskRef {
        self.0.deref().clone()
    }

    pub(crate) fn ptr_eq(&self, other: &XTaskRef) -> bool {
        Arc::ptr_eq(&self.0, other)
    }

    pub(crate) unsafe fn init_current(init_task: XTaskRef) {
        assert!(init_task.is_init());
        #[cfg(feature = "tls")]
        xhal::arch::write_thread_pointer(init_task.tls.tls_ptr() as usize);
        let ptr = Arc::into_raw(init_task);
        unsafe {
            xhal::cpu::set_current_task_ptr(ptr);
        }
    }

    pub(crate) unsafe fn set_current(prev: Self, next: XTaskRef) {
        let Self(arc) = prev;
