...
#[unsafe(no_mangle)]
#[linkage = "weak"]
static __X_TASK_EXT_SIZE: usize = 0;

#[unsafe(no_mangle)]
#[linkage = "weak"]
static __X_TASK_EXT_ALIGN: usize = 0;

#[unsafe(no_mangle)]
#[linkage = "weak"]
fn __x_task_ext_drop(_data: *mut u8) {}

/// A wrapper of pointer to the task extended data.
pub(crate) struct XTaskExt {
    ptr: *mut u8,
}
...
impl XTaskExt {
    ...
    pub fn size() -> usize {
        unsafe extern "C" {
            static __X_TASK_EXT_SIZE: usize;
        }
        unsafe { __X_TASK_EXT_SIZE }
    }

    /// Returns the expected alignment of the task extended structure.
    pub fn align() -> usize {
        unsafe extern "C" {
            static __X_TASK_EXT_ALIGN: usize;
        }
        unsafe { __X_TASK_EXT_ALIGN }
    }
...
impl Drop for XTaskExt {
    fn drop(&mut self) {
        if !self.ptr.is_null() {
            unsafe extern "C" {
                fn __x_task_ext_drop(data: *mut u8);
            }
            unsafe { __x_task_ext_drop(self.ptr) };
...
/// use xtask::{def_task_ext, TaskExtRef, TaskInner};
///
...
///
/// let task = xtask::spawn_task(inner);
...
macro_rules! def_task_ext {
    ($task_ext_struct:ty) => {
        #[unsafe(no_mangle)]
        static __X_TASK_EXT_SIZE: usize = ::core::mem::size_of::<$task_ext_struct>();

        #[unsafe(no_mangle)]
        static __X_TASK_EXT_ALIGN: usize = ::core::mem::align_of::<$task_ext_struct>();
