...
#[doc(cfg(feature = "multitask"))]
pub use crate::task_ext::{TaskExtMut, TaskExtRef, XTaskExtIf};
#[doc(cfg(feature = "multitask"))]
pub use crate::wait_queue::WaitQueue;

/// The reference type of a task.
pub type XTaskRef = Arc<XTask>;

/// The weak reference type of a task.
pub type WeakXTaskRef = Weak<XTask>;

pub use crate::task::TaskState;

/// The wrapper type for [`cpumask::CpuMask`] with SMP configuration.
pub type XCpuMask = cpumask::CpuMask<{ xconfig::SMP }>;

/// Global task table for managing all tasks by their TaskId
#[cfg(feature = "multitask")]
static TASK_TABLE: SpinNoIrq<WeakMap<u64, WeakXTaskRef>> = SpinNoIrq::new(WeakMap::new());

cfg_if::cfg_if! {
    if #[cfg(feature = "sched_rr")] {
        const MAX_TIME_SLICE: usize = 5;
        pub(crate) type XTask = xsched::RRTask<TaskInner, MAX_TIME_SLICE>;
        pub(crate) type Scheduler = xsched::RRScheduler<TaskInner, MAX_TIME_SLICE>;
    } else if #[cfg(feature = "sched_cfs")] {
        pub(crate) type XTask = xsched::CFSTask<TaskInner>;
        pub(crate) type Scheduler = xsched::CFScheduler<TaskInner>;
    } else {
        // If no scheduler features are set, use FIFO as the default.
        pub(crate) type XTask = xsched::FifoTask<TaskInner>;
        pub(crate) type Scheduler = xsched::FifoScheduler<TaskInner>;
    }
}
...
pub fn spawn_task(task: TaskInner) -> XTaskRef {
...
pub fn spawn_raw<F>(f: F, name: String, stack_size: usize) -> XTaskRef
where
...
/// [`xconfig::TASK_STACK_SIZE`].
///
/// Returns the task reference.
pub fn spawn<F>(f: F) -> XTaskRef
where
    F: FnOnce() + Send + 'static,
{
    spawn_raw(f, "".into(), xconfig::TASK_STACK_SIZE)
}
...
/// [`XCpuMask`] is used to specify the CPU affinity.
/// Returns `true` if the affinity is set successfully.
pub fn set_affinity(task: &XTaskRef, cpumask: XCpuMask) -> bool {
    if cpumask.is_empty() {
        false
    } else {
...
pub fn sleep(dur: core::time::Duration) {
    sleep_until(xhal::time::wall_time() + dur);
}
...
#[cfg(feature = "multitask")]
pub fn register_task(task_ref: &XTaskRef) {
...
///
/// Returns `Some(WeakXTaskRef)` if the task exists and is still alive (weak can still
/// fail to upgrade later), or `None` if the task doesn't exist.
///
/// This is similar to the `current()` function but works for any task ID.
#[cfg(feature = "multitask")]
pub fn get_task_by_id(task_id: TaskId) -> Option<WeakXTaskRef> {
...
pub fn with_task<R>(id: TaskId, f: impl FnOnce(&XTaskRef) -> R) -> Option<R> {
    if id.as_u64() == 0 {
