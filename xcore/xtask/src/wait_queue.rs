...
use crate::{CurrentTask, XTaskRef, current_run_queue, select_run_queue};
...
pub struct WaitQueue {
    queue: SpinNoIrq<VecDeque<XTaskRef>>,
}

pub(crate) type WaitQueueGuard<'a> = SpinNoIrqGuard<'a, VecDeque<XTaskRef>>;
...
impl WaitQueue {
    ...
    pub fn wait_timeout(&self, dur: core::time::Duration) -> bool {
        let mut rq = current_run_queue::<NoPreemptIrqSave>();
        let curr = crate::current();
        let deadline = xhal::time::wall_time() + dur;
        debug!(
        ...
        F: FnMut() -> bool,
    {
        ...
        loop {
            let mut rq = current_run_queue::<NoPreemptIrqSave>();
            if xhal::time::wall_time() >= deadline {
                break;
            }
            let wq = self.queue.lock();
            if condition() {
    ...
    /// If `resched` is true, the current task will be preempted when the
    /// preemption is enabled.
    pub fn notify_all_if(&self, resched: bool, mut pred: impl FnMut(&XTaskRef) -> bool) {
        let mut wq = self.queue.lock();
        wq.retain(|task| {
            if pred(task) {
    ...
    /// If `resched` is true, the current task will be preempted when the
    /// preemption is enabled.
    pub fn notify_task(&mut self, resched: bool, task: &XTaskRef) -> bool {
        let mut wq = self.queue.lock();
...
fn unblock_one_task(task: XTaskRef, resched: bool) {
