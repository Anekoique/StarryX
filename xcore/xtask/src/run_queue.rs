...
use xsched::BaseScheduler;

use xhal::cpu::this_cpu_id;

use crate::task::{CurrentTask, TaskState};
use crate::task_ext::__XTaskExtIf_mod;
use crate::wait_queue::WaitQueueGuard;
use crate::{Scheduler, TaskInner, WaitQueue, XCpuMask, XTaskRef};
...
percpu_static! {
    RUN_QUEUE: LazyInit<XRunQueue> = LazyInit::new(),
    EXITED_TASKS: VecDeque<XTaskRef> = VecDeque::new(),
    WAIT_FOR_EXIT: WaitQueue = WaitQueue::new(),
    IDLE_TASK: LazyInit<XTaskRef> = LazyInit::new(),
    /// Stores the weak reference to the previous task that is running on this CPU.
    #[cfg(feature = "smp")]
    PREV_TASK: Weak<crate::XTask> = Weak::new(),
}
...
static mut RUN_QUEUES: [MaybeUninit<&'static mut XRunQueue>; xconfig::SMP] =
    [ARRAY_REPEAT_VALUE; xconfig::SMP];
#[allow(clippy::declare_interior_mutable_const)] // It's ok because it's used only for initialization `RUN_QUEUES`.
const ARRAY_REPEAT_VALUE: MaybeUninit<&'static mut XRunQueue> = MaybeUninit::uninit();
...
/// ## Returns
///
/// * [`CurrentRunQueueRef`] - a static reference to the current [`XRunQueue`].
#[inline(always)]
...
///
#[cfg(feature = "smp")]
// The modulo operation is safe here because `xconfig::SMP` is always greater than 1 with "smp" enabled.
#[allow(clippy::modulo_one)]
#[inline]
fn select_run_queue_index(cpumask: XCpuMask) -> usize {
    ...
    loop {
        let index = RUN_QUEUE_INDEX.fetch_add(1, Ordering::SeqCst) % xconfig::SMP;
...
/// ## Returns
///
/// A reference to the `XRunQueue` corresponding to the provided index.
///
/// ## Panics
///
...
#[cfg(feature = "smp")]
#[inline]
fn get_run_queue(index: usize) -> &'static mut XRunQueue {
...
/// ## Returns
///
/// * [`XRunQueueRef`] - a static reference to the selected [`XRunQueue`] (current or remote).
///
/// ## TODO
///
...
///
#[inline]
pub(crate) fn select_run_queue<G: BaseGuard>(task: &XTaskRef) -> XRunQueueRef<'static, G> {
    let irq_state = G::acquire();
    #[cfg(not(feature = "smp"))]
    {
        let _ = task;
    ...
    #[cfg(feature = "smp")]
    {
        ...
        let index = if xconfig::PLATFORM == "riscv64-visionfive2" {
            1
        } else {
...
/// [`XRunQueue`] represents a run queue for global system or a specific CPU.
pub(crate) struct XRunQueue {
    /// The ID of the CPU this run queue is associated with.
    cpu_id: usize,
    /// The core scheduler of this run queue.
    /// Since irq and preempt are preserved by the kernel guard hold by `XRunQueueRef`,
...
///
/// Note:
/// [`XRunQueueRef`] is used to get a reference to the run queue on current CPU
...
pub(crate) struct XRunQueueRef<'a, G: BaseGuard> {
    inner: &'a mut XRunQueue,
    state: G::State,
    _phantom: core::marker::PhantomData<G>,
}

impl<G: BaseGuard> Drop for XRunQueueRef<'_, G> {
    fn drop(&mut self) {
...
impl<G: BaseGuard> XRunQueueRef<'_, G> {
    /// Adds a task to the scheduler.
    ///
    /// This function is used to add a new task to the scheduler.
    pub fn add_task(&mut self, task: XTaskRef) {
        debug!(
    ...
    pub fn unblock_task(&mut self, task: XTaskRef, resched: bool) {
...
impl<G: BaseGuard> CurrentRunQueueRef<'_, G> {
    ...
    #[cfg(feature = "smp")]
    pub fn migrate_current(&mut self, migration_task: XTaskRef) {
        let curr = &self.current_task;
    ...
    #[cfg(feature = "irq")]
    pub fn sleep_until(&mut self, deadline: xhal::time::TimeValue) {
        let curr = &self.current_task;
        debug!("task sleep: {}, deadline={:?}", curr.id_name(), deadline);
        assert!(curr.is_running());
        assert!(!curr.is_idle());

        let now = xhal::time::wall_time();
        if now < deadline {
...
impl XRunQueue {
    ...
    fn new(cpu_id: usize) -> Self {
        let gc_task = TaskInner::new(gc_entry, "gc".into(), xconfig::TASK_STACK_SIZE).into_arc();
        // gc task should be pinned to the current CPU.
        gc_task.set_cpumask(XCpuMask::one_shot(cpu_id));
    ...
    fn resched(&mut self) {
        ...
        if next.cpumask().get(xhal::cpu::this_cpu_id()) {
    ...
    fn switch_to(&mut self, prev_task: CurrentTask, next_task: XTaskRef) {
        ...
        if !matches!(prev_task.name(), "main" | "gc" | "idle" | "migration-task") {
            crate_interface::call_interface!(XTaskExtIf::switch_to_task);
        }
        ...
        if !matches!(
            ...
            crate_interface::call_interface!(XTaskExtIf::switch_from_task);
        }
    }
}
...
#[cfg(feature = "smp")]
pub(crate) fn migrate_entry(migrated_task: XTaskRef) {
...
#[cfg(feature = "smp")]
pub(crate) fn migrate_current(task: XTaskRef) {
    if current_run_queue::<NoPreemptIrqSave>().current_task.id() != task.id()
        || task.cpumask().get(xhal::cpu::this_cpu_id())
    {
        return;
    }
...
pub(crate) fn init() {
    ...
    idle_task.set_cpumask(XCpuMask::one_shot(cpu_id));
    ...
    RUN_QUEUE.with_current(|rq| {
        rq.init_once(XRunQueue::new(cpu_id));
    });
    unsafe {
