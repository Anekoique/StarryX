...
impl InterfaceWrapper {
    fn new(name: &'static str, dev: XNetDevice, ether_addr: EthernetAddress) -> Self {
...
impl Device for DeviceWrapper {
    ...
    fn receive(&mut self, _timestamp: Instant) -> Option<(Self::RxToken<'_>, Self::TxToken<'_>)> {
        ...
        Some((XNetRxToken(&self.inner, rx_buf), XNetTxToken(&self.inner)))
    }
...
struct XNetRxToken<'a>(&'a RefCell<XNetDevice>, NetBufPtr);
struct XNetTxToken<'a>(&'a RefCell<XNetDevice>);

impl RxToken for XNetRxToken<'_> {
...
impl TxToken for XNetTxToken<'_> {
...
pub(crate) fn init(net_dev: XNetDevice) {
