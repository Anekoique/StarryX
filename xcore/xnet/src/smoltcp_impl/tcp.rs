...
use xhal::time::current_ticks;
use xio::{PollState, Read, Write};
use xsync::Mutex;
...
impl TcpSocket {
    ...
    /// [`Err(NotConnected)`](XError::NotConnected) if not connected.
    #[inline]
    ...
    /// to be retried, an error with kind  [`Err(WouldBlock)`](XError::WouldBlock) is
    /// returned.
    #[inline]
    ...
    /// Receives data from the socket, stores it in the given buffer.
    ///
    /// It will return [`Err(Timeout)`](XError::Timeout) if expired.
...
impl TcpSocket {
    ...
    /// returns [`Err(WouldBlock)`](XError::WouldBlock).
        ...
        F: FnMut() -> NetResult<T>,
    {
        if self.is_nonblocking() {
            ...
            loop {
                if xtask::current().is_interrupted() {
                    xtask::current().set_interrupted(false);
                ...
                match f() {
                    Ok(t) => return Ok(t),
                    Err(NetError::EAGAIN) => xtask::yield_now(),
                    Err(e) => return Err(e),
                }
            }
        }
...
impl Read for TcpSocket {
    fn read(&mut self, buf: &mut [u8]) -> xerrno::XResult<usize> {
        self.recv(buf).map_err(net_error_to_axio)
    }
}

impl Write for TcpSocket {
    fn write(&mut self, buf: &[u8]) -> xerrno::XResult<usize> {
        self.send(buf).map_err(net_error_to_axio)
    }

    fn flush(&mut self) -> xerrno::XResult {
