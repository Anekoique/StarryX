...
//! categories: [`XNetDevice`], [`XBlockDevice`], and [`XDisplayDevice`].
//!
//! # Concepts
//!
...
//!   time by corresponding cargo features. For example, [`XNetDevice`] will be
...
//!   `Box<dyn Trait>`. For example, [`XNetDevice`] will be [`Box<dyn NetDriverOps>`].
...
//!   features, a dummy struct is used for [`XNetDevice`].
...
//!
//! [`VirtioNetDev`]: xdriver_virtio::VirtIoNetDev
//! [`Box<dyn NetDriverOps>`]: xdriver_net::NetDriverOps
...
use self::prelude::*;
pub use self::structs::{XDeviceContainer, XDeviceEnum};

#[cfg(feature = "block")]
pub use self::structs::XBlockDevice;
#[cfg(feature = "display")]
pub use self::structs::XDisplayDevice;
#[cfg(feature = "net")]
pub use self::structs::XNetDevice;
...
pub struct AllDevices {
    ...
    #[cfg(feature = "net")]
    pub net: XDeviceContainer<XNetDevice>,
    /// All block device drivers.
    #[cfg(feature = "block")]
    pub block: XDeviceContainer<XBlockDevice>,
    /// All graphics device drivers.
    #[cfg(feature = "display")]
    pub display: XDeviceContainer<XDisplayDevice>,
}
...
impl AllDevices {
    ...
    #[allow(dead_code)]
    fn add_device(&mut self, dev: XDeviceEnum) {
        match dev {
            #[cfg(feature = "net")]
            XDeviceEnum::Net(dev) => self.net.push(dev),
            #[cfg(feature = "block")]
            XDeviceEnum::Block(dev) => self.block.push(dev),
            #[cfg(feature = "display")]
            XDeviceEnum::Display(dev) => self.display.push(dev),
        }
    }
}
