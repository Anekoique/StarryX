...
use xdriver_base::{BaseDriverOps, DevResult, DeviceType};
use xdriver_virtio::{BufferDirection, PhysAddr, VirtIoHal};
use xhal::mem::{phys_to_virt, virt_to_phys};

use crate::{XDeviceEnum, drivers::DriverProbe};

cfg_if! {
    if #[cfg(bus = "pci")] {
        use xdriver_pci::{PciRoot, DeviceFunction, DeviceFunctionInfo};
        type VirtIoTransport = xdriver_virtio::PciTransport;
    } else if #[cfg(bus =  "mmio")] {
        type VirtIoTransport = xdriver_virtio::MmioTransport;
    }
}
...
pub trait VirtIoDevMeta {
    ...
    fn try_new(transport: VirtIoTransport) -> DevResult<XDeviceEnum>;
}
...
cfg_if! {
    if #[cfg(net_dev = "virtio-net")] {
        ...
        impl VirtIoDevMeta for VirtIoNet {
            ...
            type Device = xdriver_virtio::VirtIoNetDev<VirtIoHalImpl, VirtIoTransport, 64>;

            fn try_new(transport: VirtIoTransport) -> DevResult<XDeviceEnum> {
                Ok(XDeviceEnum::from_net(Self::Device::try_new(transport)?))
            }
        }
    }
}
...
cfg_if! {
    if #[cfg(block_dev = "virtio-blk")] {
        ...
        impl VirtIoDevMeta for VirtIoBlk {
            ...
            type Device = xdriver_virtio::VirtIoBlkDev<VirtIoHalImpl, VirtIoTransport>;

            fn try_new(transport: VirtIoTransport) -> DevResult<XDeviceEnum> {
                Ok(XDeviceEnum::from_block(Self::Device::try_new(transport)?))
            }
        }
    }
}
...
cfg_if! {
    if #[cfg(display_dev = "virtio-gpu")] {
        ...
        impl VirtIoDevMeta for VirtIoGpu {
            ...
            type Device = xdriver_virtio::VirtIoGpuDev<VirtIoHalImpl, VirtIoTransport>;

            fn try_new(transport: VirtIoTransport) -> DevResult<XDeviceEnum> {
                Ok(XDeviceEnum::from_display(Self::Device::try_new(transport)?))
            }
        }
    }
}
...
impl<D: VirtIoDevMeta> DriverProbe for VirtIoDriver<D> {
    #[cfg(bus = "mmio")]
    fn probe_mmio(mmio_base: usize, mmio_size: usize) -> Option<XDeviceEnum> {
        let base_vaddr = phys_to_virt(mmio_base.into());
        if let Some((ty, transport)) =
            xdriver_virtio::probe_mmio_device(base_vaddr.as_mut_ptr(), mmio_size)
            && ty == D::DEVICE_TYPE
        {
    ...
    fn probe_pci(
        ...
        if let Some((ty, transport)) =
            xdriver_virtio::probe_pci_device::<VirtIoHalImpl>(root, bdf, dev_info)
        {
            if ty == D::DEVICE_TYPE {
                match D::try_new(transport) {
                    Ok(dev) => return Some(dev),
