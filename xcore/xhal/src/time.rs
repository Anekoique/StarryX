...
///
/// Re-exports `xconfig::devices::TIMER_FREQUENCY` so callers (e.g. the
/// vDSO `mult/shift` derivation) don't have to import `xconfig` directly.
...
pub const fn timer_frequency() -> u64 {
    ...
    {
        xconfig::devices::TIMER_FREQUENCY as u64
    }
    #[cfg(not(any(
        platform_family = "riscv64-qemu-virt",
        platform_family = "riscv64-visionfive2",
