...
fn gen_linker_script(arch: &str, platform: &str) -> Result<()> {
    ...
    let ld_content = ld_content.replace(
        "%KERNEL_BASE%",
        &format!("{:#x}", xconfig::plat::KERNEL_BASE_VADDR),
    );
    let ld_content = ld_content.replace(
        "%KERNEL_BASE_PADDR%",
        &format!("{:#x}", xconfig::plat::KERNEL_BASE_PADDR),
    );
    let ld_content = ld_content.replace("%SMP%", &format!("{}", xconfig::SMP));

    // target/<target_triple>/<mode>/build/xhal-xxxx/out
