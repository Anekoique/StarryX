/* Modified from glibc: io/tst-copy_file_range.c for oscomp.
   ...
   #define die(fmt_, ...) \
     ...
     {                            \
       if ((a_) != (b_)) {        \
         fprintf(stderr, "compare failed: %s != %s (%lld != %lld)\n", \
                 #a_, #b_, (long long)(a_), (long long)(b_));           \
         die("compare failed");   \
       }                          \
     }
   ...
   static void simple_file_copy(void) {
     ...
     TEST_COMPARE(copy_file_range(infd, pinoff, outfd, poutoff, length, 0),
                  length);
    printf("copy_file_range length to copy: %d\n", length);
    ssize_t ret = copy_file_range(infd, pinoff, outfd, poutoff, length, 0);
    printf("copy_file_range returned %zd\n", ret);
    TEST_COMPARE(ret, length);
                  
    off_t cur_in_offset = xlseek(infd, 0, SEEK_CUR);
    printf("current in offset after copy_file_range: %lld, expected: %d\n", (long long)cur_in_offset, do_inoff ? (2 + length) : (3 + length));
     if (do_inoff) {
