...
use crate::task::{get_process, get_process_group, with_process, with_xthread};
...
pub fn sys_gettid() -> LinuxResult<isize> {
    Ok(xtask::current().id().as_u64() as _)
}
...
pub fn sys_setsid() -> LinuxResult<isize> {
    with_process(|process| {
        ...
        if current_group.pgid() == process.pid() {
            return Err(xerrno::LinuxError::EPERM);
        }

        if let Some((session, _group)) = process.create_session() {
            Ok(session.sid() as _)
        } else {
            Err(xerrno::LinuxError::EPERM)
        }
    })
}
...
pub fn sys_arch_prctl(
    ...
    with_uspace(|uspace| {
        let code = ArchPrctlCode::try_from(code).map_err(|_| xerrno::LinuxError::EINVAL)?;
        ...
        match code {
            ...
            ArchPrctlCode::SetCpuid => Err(xerrno::LinuxError::ENODEV),
        }
    })
}
