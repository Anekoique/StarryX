...
use crate::task::{XThread, get_process, get_process_group, get_thread, have_signals, with_uspace};
...
pub fn sys_sched_getaffinity(
    ...
    with_task(pid.into(), |task| {
        ...
        for cpu_id in 0..xconfig::SMP.min(len * 8) {
...
pub fn sys_nanosleep(req: UserConstPtr<timespec>, rem: UserPtr<timespec>) -> LinuxResult<isize> {
    with_uspace(|uspace| {
        ...
        let actual = sleep(xhal::time::monotonic_time, dur);
...
pub fn sys_clock_nanosleep(
    ...
    let clock = match clock_id as u32 {
        CLOCK_MONOTONIC => xhal::time::monotonic_time,
        CLOCK_REALTIME => xhal::time::wall_time,
        _ => {
