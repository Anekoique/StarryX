...
use xerrno::{LinuxError, XResult};
use xio::{BufReader, Read};
use xsync::Mutex;
use xvfs::VfsResult;
...
fn console_read_bytes(buf: &mut [u8]) -> XResult<usize> {
    let len = xhal::console::read_bytes(buf);
    Ok(len)
}

fn console_write_bytes(buf: &[u8]) -> XResult<usize> {
    xhal::console::write_bytes(buf);
    Ok(buf.len())
}
...
impl Read for Stdin {
    ...
    fn read(&mut self, buf: &mut [u8]) -> XResult<usize> {
        let mut read_len = 0;
