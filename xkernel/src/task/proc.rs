...
use xsync::Mutex;
use xtask::{TaskExtRef, TaskInner, WaitQueue, XTaskExtIf, current};
...
impl XProcess {
    pub fn new(
        ...
        Self {
            ...
            uspace,
            ns: XNamespace::new_thread_local(),
...
impl XTaskExtIf for XTaskExtImpl {
    fn switch_to_task() {}
...
struct XNamespaceImpl;
#[crate_interface::impl_interface]
impl XNamespaceIf for XNamespaceImpl {
    fn current_namespace_base() -> *mut u8 {
        static KERNEL_NS_BASE: Once<usize> = Once::new();
        let current = xtask::current();
        if unsafe { current.task_ext_ptr() }.is_null() {
            return *(KERNEL_NS_BASE.call_once(|| {
                let global_ns = XNamespace::global();
